       ...
       while( i < ( 5 + t_counter) && Found == 0)
       {
         ...
         if(strcmp(current_id, symbol_table[i]->id) == 0)
         {
           ...
           for(int j = 0; j < (5 + t_counter); j++)
           {
             ...
             {
                symbol_table[j]->type_number = goutideke;
             }
           }
           Found = 1;
          }
...
void assign_expr(struct exprNode* expr, int type)
{
   ...
   if(expr -> tag == PRIMARY)
   {
     assign_primary(expr -> primary,goutideke);
   }
   else if(expr -> tag = EXPR)
   {
     assign_expr(expr->leftOperand, goutideke);
     assign_expr(expr->rightOperand, goutideke);
   }
...
int Parser::analysis_assign(struct assign_stmtNode* assign_stmt)
{
     ...
     if (rhs_number >= 0)
    {
                ...
                {
                   int goutideke = symbol_table[index]->type_number;
                    int temp2 = 5;
                    while(temp2 < (5+t_counter))
                    {
                     if(symbol_table[temp2]->type_number == goutideke)
                     {
